...
<div class="card">
  ...
  ${msgs.filter(m => m.role !== 'system' && !m.content?.startsWith('[TOOL RESULT') && !m.content?.startsWith('[TOOL ERROR')).slice(-20).reverse().map(m => {
    ...
    if (m.role === 'assistant') {
      ...
      if (jsonIdx > 0) {
        ...
        displayHtml = '<em style="color:#8b949e">working...</em>';
      } else {
