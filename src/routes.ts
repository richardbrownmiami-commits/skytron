  ...
  if (url.pathname === "/think" && req.method === "POST") {
    try {
      ...
      try {
        ...
        if (mp?.content) memoryPack = "\n\n## MEMORY PACK (recall rules — FOLLOW THIS when asked about past work or memory):\n" + mp.content.slice(0, 4000) + "\n\nIMPORTANT: The memory pack rules override the 'under 20 words' rule for recall questions. Use the structured format (date, topic, status, confirmed, caution, next) when answering about past work.\n";
      } catch {}
