...
<script>async function save(){const s={};document.querySelectorAll('input[type=checkbox]').forEach(c=>{s[c.id]=c.checked});await fetch('/cron/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(s)});const e=document.getElementById('saved');e.style.display='block';setTimeout(()=>e.style.display='none',2000)}</script></body></html>`, { headers: { "Content-Type": "text/html;charset=utf-8", "Cache-Control": "no-cache, no-store, must-revalidate" } });
  ...
  if (url.pathname === "/brain/insight") {
    ...
    if (accept.includes("text/html") || accept.includes("*/*")) {
      return new Response(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Skytron — Monitor</title><script>!function(){try{var t=JSON.parse(localStorage.getItem('st_theme'));if(t){var r=document.documentElement;Object.entries(t).forEach(function(e){r.style.setProperty('--'+e[0],e[1])})}}catch(e){}}()</script>
<style>:root{--bg:#080c18;--surface:rgba(15,23,42,.65);--border:rgba(99,102,241,.15);--glow:rgba(99,102,241,.08);--primary:#6366f1;--accent:#8b5cf6;--green:#22c55e;--red:#ef4444;--amber:#eab308;--text:#e2e8f0;--muted:#64748b;--radius:14px}@keyframes bgShift{0%{background-position:0 50%}50%{background-position:100% 50%}100%{background-position:0 50%}}@keyframes slideUp{from{opacity:0;transform:translateY(16px) scale(.97)}to{opacity:1;transform:translateY(0) scale(1)}}@keyframes countUp{from{opacity:0;transform:scale(.5)}to{opacity:1;transform:scale(1)}}@keyframes ping{0%{transform:scale(1);opacity:.8}100%{transform:scale(1.8);opacity:0}}body{min-height:100vh;background:linear-gradient(135deg,#080c18,#0d1117,#111827,#0a0a2e);background-size:400% 400%;animation:bgShift 24s ease infinite;color:var(--text);font-family:system-ui,-apple-system,sans-serif;display:flex;flex-direction:column;padding:0;overflow-x:hidden}::selection{background:var(--primary);color:#fff}.wrap{padding:12px 16px 20px;flex:1;display:flex;flex-direction:column;gap:12px;max-width:1400px;margin:0 auto;width:100%}.header{display:flex;align-items:center;justify-content:space-between;animation:slideUp .5s ease;margin-bottom:4px}.header h1{font-size:1.3rem;font-weight:700;background:linear-gradient(135deg,var(--primary),var(--accent));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}.header .sub{font-size:.75rem;color:var(--muted)}.header .last-upd{font-size:.7rem;color:var(--muted);font-family:monospace}
.cards{display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:8px;animation:slideUp .6s ease}.card{background:var(--surface);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid var(--border);border-radius:var(--radius);padding:12px 14px;position:relative;overflow:hidden;transition:all .25s ease}.card:hover{border-color:rgba(99,102,241,.3);transform:translateY(-1px)}.card .icon{font-size:1.1rem;margin-bottom:4px;opacity:.7}.card .val{font-size:1.4rem;font-weight:700;letter-spacing:-.02em;font-variant-numeric:tabular-nums}.card .lbl{font-size:.65rem;color:var(--muted);margin-top:2px;text-transform:uppercase;letter-spacing:.04em}.card .mini{position:absolute;top:10px;right:12px;width:5px;height:5px;border-radius:50%}.card .mini.green{background:var(--green);box-shadow:0 0 6px var(--green)}.card .mini.red{background:var(--red);box-shadow:0 0 6px var(--red)}.card .mini.amber{background:var(--amber);box-shadow:0 0 6px var(--amber)}
.grid{display:grid;grid-template-columns:1fr 380px;gap:10px;flex:1;min-height:0;animation:slideUp .7s ease}@media(max-width:900px){.grid{grid-template-columns:1fr}}
.panel{background:var(--surface);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid var(--border);border-radius:var(--radius);display:flex;flex-direction:column;overflow:hidden}.panel h2{font-size:.72rem;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.06em;padding:12px 14px 8px;border-bottom:1px solid var(--border);flex-shrink:0;display:flex;align-items:center;gap:5px}.panel .body{flex:1;overflow-y:auto;padding:4px 8px}.panel .body::-webkit-scrollbar{width:4px}.panel .body::-webkit-scrollbar-track{background:transparent}.panel .body::-webkit-scrollbar-thumb{background:rgba(99,102,241,.2);border-radius:4px}
.act-item{display:flex;flex-direction:column;gap:2px;padding:7px 6px;border-bottom:1px solid rgba(255,255,255,.04);font-size:.76rem;transition:background .15s;border-radius:6px;cursor:pointer}.act-item:hover{background:rgba(99,102,241,.06)}.act-item .top{display:flex;align-items:center;gap:6px}.act-item .top .dot{width:6px;height:6px;border-radius:50%;flex-shrink:0}.act-item .top .dot.done{background:var(--green);box-shadow:0 0 4px var(--green)}.act-item .top .dot.error{background:var(--red);box-shadow:0 0 4px var(--red)}.act-item .top .dot.running{background:var(--primary);box-shadow:0 0 4px var(--primary);animation:glowPulse 1.5s infinite}.act-item .top .dot.queued{background:var(--amber);box-shadow:0 0 4px var(--amber)}.act-item .top .aid{color:var(--muted);font-size:.65rem;font-family:monospace}.act-item .top .atype{color:var(--primary);font-size:.68rem;font-weight:500}.act-item .top .atask{flex:1;color:var(--text)}.act-item .top .stat{font-size:.6rem;padding:1px 5px;border-radius:3px;text-transform:uppercase}.act-item .top .stat.done{background:rgba(34,197,94,.12);color:var(--green)}.act-item .top .stat.error{background:rgba(239,68,68,.12);color:var(--red)}.act-item .top .stat.running{background:rgba(99,102,241,.12);color:var(--primary)}.act-item .top .stat.queued{background:rgba(234,179,8,.12);color:var(--amber)}.act-item .detail{font-size:.68rem;color:var(--muted);padding:3px 12px 0;line-height:1.4;display:none;white-space:pre-wrap;word-break:break-word}.act-item .detail.show{display:block}.act-item .err-text{color:var(--red)}.act-item .ok-text{color:var(--green)}
.chat-area{display:flex;flex-direction:column;height:100%}.chat-msgs{flex:1;overflow-y:auto;padding:6px 8px;display:flex;flex-direction:column;gap:5px}.chat-msgs::-webkit-scrollbar{width:4px}.chat-msgs::-webkit-scrollbar-track{background:transparent}.chat-msgs::-webkit-scrollbar-thumb{background:rgba(99,102,241,.2);border-radius:4px}.chat-bubble{max-width:92%;padding:6px 10px;border-radius:10px;font-size:.76rem;line-height:1.5;animation:slideUp .3s ease;white-space:pre-wrap;word-break:break-word}.chat-bubble.q{background:rgba(99,102,241,.15);border:1px solid rgba(99,102,241,.2);align-self:flex-end;border-bottom-right-radius:3px}.chat-bubble.a{background:rgba(15,23,42,.5);border:1px solid rgba(255,255,255,.06);align-self:flex-start;border-bottom-left-radius:3px}.chat-bubble .cb-time{font-size:.55rem;color:var(--muted);margin-top:3px;opacity:.7}.chat-thinking{display:flex;gap:3px;padding:8px 12px;align-self:flex-start}.chat-thinking span{width:4px;height:4px;border-radius:50%;background:var(--primary)}.chat-input{display:flex;gap:5px;padding:6px 8px 8px;border-top:1px solid var(--border);flex-shrink:0}.chat-input input{flex:1;padding:6px 10px;border-radius:7px;border:1px solid var(--border);background:rgba(15,23,42,.4);color:var(--text);font-size:.76rem;outline:none;transition:border-color .2s}.chat-input input:focus{border-color:var(--primary)}.chat-input input::placeholder{color:var(--muted);font-size:.7rem}.chat-input button{padding:6px 14px;border-radius:7px;border:none;background:linear-gradient(135deg,var(--primary),var(--accent));color:#fff;font-size:.76rem;font-weight:600;cursor:pointer;transition:all .2s;flex-shrink:0}.chat-input button:hover{opacity:.9}.chat-input button:disabled{opacity:.4;cursor:not-allowed}
...
let lastData=null,expandedIds=new Set();
...
const parts=[];if(a.error)parts.push(a.error);if(a.result)parts.push(a.result);if(a.input)parts.push(a.type==='chat'?'User: '+a.input.slice(0,150):'Goal: '+a.input.slice(-150));const detail=parts.join(' | ')
...
const chatMsgs=$('chatMsgs');let thinking=false
function addMsg(text,role){const d=document.createElement('div');d.className='chat-bubble '+(role==='user'?'q':'a');const t=document.createElement('div');t.className='cb-time';t.textContent=new Date().toLocaleTimeString();d.innerHTML=esc(text);d.appendChild(t);chatMsgs.appendChild(d);chatMsgs.scrollTop=chatMsgs.scrollHeight}
...
</script></body></html>`, { headers: { "Content-Type": "text/html;charset=utf-8", "Cache-Control": "no-cache, no-store, must-revalidate" } });
    }
  ...
  if (url.pathname === "/think" && req.method === "POST") {
    try {
      ...
      if (mode === "discussion") {
        ...
        await storeMemory(env.DB, "assistant", "Skytron had trouble responding. Try again? (" + errMsg + ")", conversationId);
