...
export async function handleFetch(req, env, ctx, CHAT_HTML) {
  ...
  if (url.pathname === "/cron/settings") {
    ...
    const d = { enabled: true, log_tick: false, idle_cycle: true, health_check: true, slot_self_improve: true, slot_test: true, slot_research: true, slot_housekeep: true, idle_project: true, tool_dispatch: true, process_actions: true, stuck_recovery: true, process_agents: true, daily_cleanup: true, wake_up: true, task_web_search: true, task_memory_search: true, task_learn: true, task_db_query: true, task_review_code: true, astral_active: false, astral_interval: "120", emergency_repair: true, loop_detection: true, file_index: true, consolidation: true };
    const set = (k) => s[k] !== undefined ? s[k] : d[k];
    const raw = (k) => rawS[k] !== undefined ? rawS[k] : String(d[k]);
    const c = (k) => set(k) ? "checked" : "";
    return new Response(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Cron Settings</title><style>:root{--bg:#080c18;--surface:rgba(15,23,42,.65);--border:rgba(99,102,241,.15);--glow:rgba(99,102,241,.08);--primary:#6366f1;--accent:#8b5cf6;--green:#22c55e;--red:#ef4444;--amber:#eab308;--text:#e2e8f0;--muted:#64748b;--radius:14px}*{margin:0;padding:0;box-sizing:border-box}body{min-height:100vh;background:linear-gradient(135deg,#080c18,#0d1117,#111827,#0a0a2e);background-size:400% 400%;animation:bgShift 24s ease infinite;color:var(--text);font-family:system-ui,-apple-system,sans-serif;padding:20px 16px;overflow-x:hidden}@keyframes bgShift{0%{background-position:0 50%}50%{background-position:100% 50%}100%{background-position:0 50%}}@keyframes slideUp{from{opacity:0;transform:translateY(16px) scale(.97)}to{opacity:1;transform:translateY(0) scale(1)}}@keyframes glowPulse{0%,100%{opacity:.3}50%{opacity:.7}}::selection{background:var(--primary);color:#fff}.wrap{max-width:720px;margin:0 auto;animation:slideUp .6s ease}.header{display:flex;align-items:center;justify-content:space-between;margin-bottom:16px}.header h1{font-size:1.3rem;font-weight:700;background:linear-gradient(135deg,var(--primary),var(--accent));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}.header .sub{font-size:.75rem;color:var(--muted);-webkit-text-fill-color:var(--muted)}.section{background:var(--surface);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid var(--border);border-radius:var(--radius);padding:14px 16px;margin-bottom:12px;position:relative;overflow:hidden;transition:all .3s ease}.section:hover{border-color:rgba(99,102,241,.3)}.section h2{font-size:.75rem;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:6px}.sec-icon{font-size:.85rem;opacity:.7}.row{display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid rgba(255,255,255,.04)}.row:last-child{border-bottom:none}.label{flex:1}.label .name{font-size:.82rem;color:var(--text);font-weight:500}.label .desc{font-size:.68rem;color:var(--muted);margin-top:2px}.switch{position:relative;width:40px;height:22px;flex-shrink:0;margin-left:12px}input{opacity:0;width:0;height:0}.slider{position:absolute;cursor:pointer;top:0;left:0;right:0;bottom:0;background-color:rgba(255,255,255,.08);transition:.25s;border-radius:22px;border:1px solid rgba(255,255,255,.06)}.slider:before{position:absolute;content:"";height:16px;width:16px;left:3px;bottom:2.5px;background-color:var(--muted);transition:.25s;border-radius:50%}input:checked+.slider{background-color:rgba(99,102,241,.3);border-color:var(--primary)}input:checked+.slider:before{background-color:var(--primary);transform:translateX(17px)}.saved{position:fixed;bottom:20px;right:20px;background:var(--surface);backdrop-filter:blur(20px);border:1px solid var(--green);border-radius:10px;padding:10px 18px;font-size:.8rem;color:var(--green);display:none;animation:slideUp .3s ease;z-index:100}.dot{display:inline-block;width:6px;height:6px;border-radius:50%;margin-right:6px}.dot.green{background:var(--green);box-shadow:0 0 8px var(--green)}.dot.amber{background:var(--amber);box-shadow:0 0 8px var(--amber)}.dot.primary{background:var(--primary);box-shadow:0 0 8px var(--primary)}</style></head><body><div class="wrap"><div class="header"><div><h1>Cron Settings</h1><div class="sub">Every cron feature — toggle on/off, read every tick</div></div><span class="saved" id="saved">✓ Saved</span></div>

<div class="section"><h2><span class="sec-icon">⚡</span> Master</h2>
<div class="row"><div class="label"><div class="name">Cron Enabled</div><div class="desc">Master switch — disables ALL cron activity when off</div></div><label class="switch"><input type="checkbox" id="enabled" `+c("enabled")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Wake-Up Heartbeat</div><div class="desc">Queue a think action every ~15min to keep Skytron awake (action needs process_actions ON to run)</div></div><label class="switch"><input type="checkbox" id="wake_up" `+c("wake_up")+` onchange="save()"><span class="slider"></span></label></div>
</div>
...
<div class="row"><div class="label"><div class="name">Stuck Recovery</div><div class="desc">Detect and requeue actions stuck in running >2min; kill at 15min hard limit</div></div><label class="switch"><input type="checkbox" id="stuck_recovery" `+c("stuck_recovery")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Loop Detection</div><div class="desc">Scan failure patterns (rate-limit, timeout, auth, empty) — escalates after 3 repeats</div></div><label class="switch"><input type="checkbox" id="loop_detection" `+c("loop_detection")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Idle Exploration</div><div class="desc">When no actions queued, creates a self_explore action every 30min (needs process_actions ON)</div></div><label class="switch"><input type="checkbox" id="idle_project" `+c("idle_project")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Source File Index</div><div class="desc">Rebuild knowledge file index from source_% entries each tick</div></div><label class="switch"><input type="checkbox" id="file_index" `+c("file_index")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Consolidation</div><div class="desc">Collect new records to scratchpad (memory → durable knowledge pipeline)</div></div><label class="switch"><input type="checkbox" id="consolidation" `+c("consolidation")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Daily Cleanup</div><div class="desc">Trim old memories (>200), logs (>1000), done actions (>500), agents (>50) — once per day</div></div><label class="switch"><input type="checkbox" id="daily_cleanup" `+c("daily_cleanup")+` onchange="save()"><span class="slider"></span></label></div>
</div>

<div class="section"><h2><span class="sec-icon">🔄</span> Idle Cycle — hourly maintenance (when no actions queued)</h2>
<div class="row"><div class="label"><div class="name">Idle LLM Cycle</div><div class="desc">Master — run hourly maintenance when idle (errors→lessons, memory summarization, stats)</div></div><label class="switch"><input type="checkbox" id="idle_cycle" `+c("idle_cycle")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Health Check</div><div class="desc">Log a BD provider health check once per hour (sub-flag of idle cycle)</div></div><label class="switch"><input type="checkbox" id="health_check" `+c("health_check")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Tool Dispatch</div><div class="desc">Actually run the tool the LLM chose during idle cycle (off = LLM decides but nothing executes)</div></div><label class="switch"><input type="checkbox" id="tool_dispatch" `+c("tool_dispatch")+` onchange="save()"><span class="slider"></span></label></div>
</div>

<div class="section"><h2><span class="sec-icon">🗺️</span> Astral Walk — autonomous agent ticks</h2>
<div class="row"><div class="label"><div class="name">Astral Active</div><div class="desc">Enable autonomous astral walk ticks — Skytron controls his own schedule</div></div><label class="switch"><input type="checkbox" id="astral_active" `+c("astral_active")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Astral Interval</div><div class="desc">Seconds between astral ticks (Skytron sets this via cron_control tool)</div></div><span style="color:var(--muted);font-size:.8rem">${raw("astral_interval")}s</span></div>
</div>

<div class="section"><h2><span class="sec-icon">📋</span> Hour Slots — round-robin task categories</h2>
<div class="row"><div class="label"><div class="name">Self-Improve</div><div class="desc">review_code, create_tool — runs on hours HH%4=0</div></div><label class="switch"><input type="checkbox" id="slot_self_improve" `+c("slot_self_improve")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Test</div><div class="desc">Run checks after changes — runs on hours HH%4=1</div></div><label class="switch"><input type="checkbox" id="slot_test" `+c("slot_test")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Research</div><div class="desc">web_search, memory_search — runs on hours HH%4=2</div></div><label class="switch"><input type="checkbox" id="slot_research" `+c("slot_research")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Housekeep</div><div class="desc">learn, db_query — runs on hours HH%4=3</div></div><label class="switch"><input type="checkbox" id="slot_housekeep" `+c("slot_housekeep")+` onchange="save()"><span class="slider"></span></label></div>
</div>

<div class="section"><h2><span class="sec-icon">🎯</span> Per-Task Toggles — fine-grain tool control</h2>
<div class="row"><div class="label"><div class="name">Web Search</div><div class="desc">Search the web for info (via Tavily/Tinyfish)</div></div><label class="switch"><input type="checkbox" id="task_web_search" `+c("task_web_search")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Memory Search</div><div class="desc">Search brain memory and knowledge</div></div><label class="switch"><input type="checkbox" id="task_memory_search" `+c("task_memory_search")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Learn</div><div class="desc">Store facts, lessons, and journals</div></div><label class="switch"><input type="checkbox" id="task_learn" `+c("task_learn")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">DB Query</div><div class="desc">Run SQL SELECT queries</div></div><label class="switch"><input type="checkbox" id="task_db_query" `+c("task_db_query")+` onchange="save()"><span class="slider"></span></label></div>
<div class="row"><div class="label"><div class="name">Review Code</div><div class="desc">Self-audit, review source files</div></div><label class="switch"><input type="checkbox" id="task_review_code" `+c("task_review_code")+` onchange="save()"><span class="slider"></span></label></div>
</div>

<div class="section"><h2><span class="sec-icon">📊</span> Logging</h2>
<div class="row"><div class="label"><div class="name">Log Tick</div><div class="desc">Log every cron tick to brain_logs (off by default — noisy)</div></div><label class="switch"><input type="checkbox" id="log_tick" `+c("log_tick")+` onchange="save()"><span class="slider"></span></label></div>
</div>
...
<script>async function save(){const s={};document.querySelectorAll('input[type=checkbox]').forEach(c=>{s[c.id]=c.checked});await fetch('/cron/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(s)});const e=document.getElementById('saved');e.style.display='block';setTimeout(()=>e.style.display='none',2000)}</script></body></html>`, { headers: { "Content-Type": "text/html;charset=utf-8", "Cache-Control": "no-cache, no-store, must-revalidate" } });
  ...
  if (url.pathname === "/brain/source") {
    return json({
      ...
      endpoints: ["/think","/status","/skytronchat","/","/brain/history","/brain/memory","/brain/memory/search","/brain/knowledge","/brain/prompt","/brain/prompt/reset","/brain/repair","/brain/logs","/brain/vectorize","/brain/introspect","/brain/insight","/brain/source","/brain/agents","/cron/settings"],
  ...
  if (url.pathname === "/brain/introspect") {
    ...
    if (q) {
      ...
      let hours = 2, range = "last 2 hours";
      if (/last hour|1 hour|60 min|recently/i.test(msg)) { hours = 1; range = "last hour"; }
      else if (/today|past 24|last 24|\bday\b/i.test(msg)) { hours = 24; range = "today"; }
      else if (/3 days?|72 hour/i.test(msg)) { hours = 72; range = "last 3 days"; }
      else if (/week|7 days?|168 hour/i.test(msg)) { hours = 168; range = "last week"; }
      else if (/month|30 days?/i.test(msg)) { hours = 720; range = "last month"; }
      else if (/all|ever|total|since|forever/i.test(msg)) { hours = 100000; range = "all time"; }

      const isErrors = /\berror\b|\bfail\b|\bstuck\b|\bproblem\b|\bissue\b|\bwrong\b|\bbreak/.test(msg);
      const isConvs = /\bconversation|\btalk\b|\bchat\b|\bdiscuss|\bsay\b|\bspeak|\buser\b|\bmessage/.test(msg);
      const isKnowledge = /\bknowledge|\blearn\b|\bfact\b|\binfo/.test(msg);
      const isTimeline = /\btimeline|\blog\b|\bhistory|\bsequence|\bhappen|\brecent/.test(msg);
      const isActions = /\btool\b|\baction\b|\bstep\b|\btask\b|\bprocess|\bdo\b/.test(msg);
      const isHealth = /\bhealth|\balive|\bworking|\bok\b|\bfine\b|\bup\b/.test(msg);
      const isAgents = /\bagent/.test(msg);
      const isMemory = /\bmemory|\bremember/.test(msg);

      const tf = `WHERE created_at > datetime('now', '-${hours} hours')`;
      const ta = `AND created_at > datetime('now', '-${hours} hours')`;
      const [actTotal, actDone, actErr, actRun, actQue, convCount, knTotal, memTotal, agentCount, recentAct, toolDist, errList, knCats] = await Promise.all([
        env.DB.prepare(`SELECT COUNT(*) as c FROM actions ${tf}`).all(),
        env.DB.prepare(`SELECT COUNT(*) as c FROM actions WHERE status='done' ${ta}`).all(),
        env.DB.prepare(`SELECT COUNT(*) as c FROM actions WHERE status='error' ${ta}`).all(),
        env.DB.prepare(`SELECT COUNT(*) as c FROM actions WHERE status='running' ${ta}`).all(),
        env.DB.prepare(`SELECT COUNT(*) as c FROM actions WHERE status='queued' ${ta}`).all(),
        env.DB.prepare(`SELECT COUNT(DISTINCT conversation_id) as c FROM brain_memory ${tf}`).all(),
        ...
        env.DB.prepare(`SELECT id, task, status, created_at FROM actions ${tf} ORDER BY created_at DESC LIMIT 15`).all(),
      ...
      const aTot = actTotal.results[0]?.c || 0, aDone = actDone.results[0]?.c || 0, aErr = actErr.results[0]?.c || 0, aRun = actRun.results[0]?.c || 0, aQue = actQue.results[0]?.c || 0, cCount = convCount.results[0]?.c || 0, kCount = knTotal.results[0]?.c || 0, mCount = memTotal.results[0]?.c || 0, aCount = agentCount.results[0]?.c || 0;
      let reply = "";
      if (isErrors) { const errs = errList.results || []; reply = aErr === 0 ? `No errors in the ${range}. Smooth sailing!` : `${aErr} error(s) in the ${range}. Latest: ${errs.slice(0, 3).map(e => `#${e.id} (${e.task}): "${(e.result||"").slice(0, 80)}"`).join("; ")}`; }
      else if (isConvs) { reply = `${cCount} conversation(s) in the ${range}. ${mCount} total messages across all time. ${aDone} actions processed.`; }
      else if (isKnowledge) { const cats = (knCats.results || []).map(r => `${r.category} (${r.c})`).join(", "); reply = `${kCount} knowledge entries total. Categories: ${cats || "none"}`; }
      else if (isTimeline) { const acts = (recentAct.results || []).slice(0, 10); reply = `Recent activity in the ${range}:\n${acts.map(a => `${(a.created_at||"").slice(11,19)} - ${a.task||"?"} [${a.status}]`).join("\n")}`; }
      else if (isHealth) { reply = `Memory: ${mCount} msgs | Knowledge: ${kCount} facts | Actions: ${aTot} (${aErr} errors, ${aRun} running) | Agents: ${aCount} | Conversations: ${cCount}. System is ${aErr > 3 ? "having issues" : aErr > 0 ? "mostly fine" : "running smoothly"}.`; }
    ...
    const recentAct = (await env.DB.prepare("SELECT id, task, status, created_at FROM actions WHERE created_at > datetime('now', '-2 hours') ORDER BY created_at DESC LIMIT 20").all()).results || [];
    ...
    return json({ summary: { total_memories: totalMem, total_knowledge: totalKn, total_actions: totalActions, conversations: convCount, running_actions: actRun, error_actions: actErr, queued_actions: actQue, agents: agentCount }, recent_activity: recentAct.map(a => ({ id: a.id, task: a.task || "?", status: a.status, time: a.created_at })), top_conversations: topConvs, activity_30d: recent, knowledge_categories: cats });
  }

  if (url.pathname === "/brain/insight") {
    const accept = req.headers.get("accept") || "";
    if (accept.includes("text/html") || accept.includes("*/*")) {
      return new Response(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Skytron — Monitor</title><script>!function(){try{var t=JSON.parse(localStorage.getItem('st_theme'));if(t){var r=document.documentElement;Object.entries(t).forEach(function(e){r.style.setProperty('--'+e[0],e[1])})}}catch(e){}}()</script><style>*{margin:0;padding:0;box-sizing:border-box}:root{--bg:#080c18;--surface:rgba(15,23,42,.65);--border:rgba(99,102,241,.15);--glow:rgba(99,102,241,.08);--primary:#6366f1;--accent:#8b5cf6;--green:#22c55e;--red:#ef4444;--amber:#eab308;--text:#e2e8f0;--muted:#64748b;--radius:14px}@keyframes bgShift{0%{background-position:0 50%}50%{background-position:100% 50%}100%{background-position:0 50%}}@keyframes glowPulse{0%,100%{opacity:.3}50%{opacity:.7}}@keyframes slideUp{from{opacity:0;transform:translateY(16px) scale(.97)}to{opacity:1;transform:translateY(0) scale(1)}}@keyframes countUp{from{opacity:0;transform:scale(.5)}to{opacity:1;transform:scale(1)}}@keyframes shimmer{0%{background-position:-200% 0}100%{background-position:200% 0}}@keyframes ping{0%{transform:scale(1);opacity:.8}100%{transform:scale(1.8);opacity:0}}body{min-height:100vh;background:linear-gradient(135deg,#080c18,#0d1117,#111827,#0a0a2e);background-size:400% 400%;animation:bgShift 24s ease infinite;color:var(--text);font-family:system-ui,-apple-system,sans-serif;display:flex;flex-direction:column;padding:0;overflow-x:hidden}::selection{background:var(--primary);color:#fff}.topnav{display:flex;gap:4px;padding:10px 16px 0;overflow-x:auto;-webkit-overflow-scrolling:touch;flex-shrink:0}.topnav a{color:var(--muted);text-decoration:none;padding:5px 12px;border-radius:8px;font-size:.75rem;white-space:nowrap;transition:all .2s;border:1px solid transparent}.topnav a:hover{color:var(--text);background:var(--surface);border-color:var(--border)}.topnav .active{color:var(--primary);background:rgba(99,102,241,.1);border-color:rgba(99,102,241,.25)}.wrap{padding:12px 16px 20px;flex:1;display:flex;flex-direction:column;gap:12px;max-width:1400px;margin:0 auto;width:100%}.header{display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;animation:slideUp .5s ease}.header h1{font-size:1.3rem;font-weight:700;background:linear-gradient(135deg,var(--primary),var(--accent));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}.header .sub{font-size:.75rem;color:var(--muted);-webkit-text-fill-color:var(--muted)}.cards{display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:10px;animation:slideUp .6s ease}.card{background:var(--surface);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid var(--border);border-radius:var(--radius);padding:14px 16px;position:relative;overflow:hidden;transition:all .3s ease}.card::before{content:'';position:absolute;inset:0;border-radius:inherit;background:radial-gradient(600px circle at var(--mx,50%) var(--my,50%),rgba(99,102,241,.06),transparent 60%);pointer-events:none;opacity:0;transition:opacity .4s}.card:hover::before{opacity:1}.card:hover{border-color:rgba(99,102,241,.3);transform:translateY(-2px)}.card .icon{font-size:1.2rem;margin-bottom:6px;opacity:.7}.card .val{font-size:1.6rem;font-weight:700;letter-spacing:-.02em;animation:countUp .6s ease;font-variant-numeric:tabular-nums}.card .lbl{font-size:.7rem;color:var(--muted);margin-top:2px;text-transform:uppercase;letter-spacing:.04em}.card .trend{font-size:.65rem;margin-top:4px;display:flex;align-items:center;gap:3px}.card .trend.up{color:var(--green)}.card .trend.down{color:var(--red)}.card .mini{position:absolute;top:12px;right:14px;width:6px;height:6px;border-radius:50%}.card .mini.green{background:var(--green);box-shadow:0 0 8px var(--green);animation:ping 2s ease infinite}.card .mini.red{background:var(--red);box-shadow:0 0 8px var(--red)}.card .mini.amber{background:var(--amber);box-shadow:0 0 8px var(--amber)}.grid{display:grid;grid-template-columns:1fr 400px;gap:12px;flex:1;min-height:0;animation:slideUp .7s ease}@media(max-width:900px){.grid{grid-template-columns:1fr}}.panel{background:var(--surface);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid var(--border);border-radius:var(--radius);display:flex;flex-direction:column;overflow:hidden}.panel h2{font-size:.75rem;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.06em;padding:14px 16px 10px;border-bottom:1px solid var(--border);flex-shrink:0;display:flex;align-items:center;gap:6px}.panel .body{flex:1;overflow-y:auto;padding:8px 12px}.panel .body::-webkit-scrollbar{width:4px}.panel .body::-webkit-scrollbar-track{background:transparent}.panel .body::-webkit-scrollbar-thumb{background:rgba(99,102,241,.2);border-radius:4px}.tl-item{display:flex;align-items:flex-start;gap:10px;padding:7px 4px;border-bottom:1px solid rgba(255,255,255,.04);font-size:.8rem;transition:background .2s;border-radius:6px}.tl-item:hover{background:rgba(99,102,241,.05)}.tl-item .dot{width:7px;height:7px;border-radius:50%;flex-shrink:0;margin-top:5px;position:relative}.tl-item .dot.done{background:var(--green);box-shadow:0 0 6px var(--green)}.tl-item .dot.error{background:var(--red);box-shadow:0 0 6px var(--red)}.tl-item .dot.running{background:var(--primary);box-shadow:0 0 6px var(--primary);animation:glowPulse 1.5s ease infinite}.tl-item .dot.queued{background:var(--amber);box-shadow:0 0 6px var(--amber)}.tl-item .time{color:var(--muted);font-size:.68rem;min-width:50px;font-family:monospace}.tl-item .task{flex:1;color:var(--text)}.tl-item .task .tid{color:var(--muted);font-size:.65rem}.tl-item .stat{font-size:.62rem;padding:1px 6px;border-radius:4px;text-transform:uppercase;letter-spacing:.03em}.tl-item .stat.done{background:rgba(34,197,94,.12);color:var(--green)}.tl-item .stat.error{background:rgba(239,68,68,.12);color:var(--red)}.tl-item .stat.running{background:rgba(99,102,241,.12);color:var(--primary)}.tl-item .stat.queued{background:rgba(234,179,8,.12);color:var(--amber)}.status-bar{display:flex;gap:12px;padding:10px 16px;border-top:1px solid var(--border);flex-wrap:wrap;flex-shrink:0}.status-item{font-size:.72rem;display:flex;align-items:center;gap:4px;color:var(--muted)}.status-item .sdot{width:6px;height:6px;border-radius:50%}.chat-area{display:flex;flex-direction:column;height:100%}.chat-msgs{flex:1;overflow-y:auto;padding:8px 10px;display:flex;flex-direction:column;gap:6px}.chat-msgs::-webkit-scrollbar{width:4px}.chat-msgs::-webkit-scrollbar-track{background:transparent}.chat-msgs::-webkit-scrollbar-thumb{background:rgba(99,102,241,.2);border-radius:4px}.chat-bubble{max-width:92%;padding:8px 12px;border-radius:12px;font-size:.8rem;line-height:1.5;animation:slideUp .3s ease;white-space:pre-wrap;word-break:break-word}.chat-bubble.q{background:rgba(99,102,241,.15);border:1px solid rgba(99,102,241,.2);align-self:flex-end;border-bottom-right-radius:4px}.chat-bubble.a{background:rgba(15,23,42,.5);border:1px solid rgba(255,255,255,.06);align-self:flex-start;border-bottom-left-radius:4px}.chat-bubble .cb-time{font-size:.6rem;color:var(--muted);margin-top:4px;opacity:.7}.chat-input{display:flex;gap:6px;padding:8px 10px 10px;border-top:1px solid var(--border);flex-shrink:0}.chat-input input{flex:1;padding:8px 12px;border-radius:8px;border:1px solid var(--border);background:rgba(15,23,42,.4);color:var(--text);font-size:.8rem;outline:none;transition:border-color .2s}.chat-input input:focus{border-color:var(--primary)}.chat-input input::placeholder{color:var(--muted);font-size:.75rem}.chat-input button{padding:8px 16px;border-radius:8px;border:none;background:linear-gradient(135deg,var(--primary),var(--accent));color:#fff;font-size:.8rem;font-weight:600;cursor:pointer;transition:all .2s;flex-shrink:0}.chat-input button:hover{opacity:.9;transform:scale(1.02)}.chat-input button:disabled{opacity:.4;cursor:not-allowed;transform:none}.chat-welcome{color:var(--muted);font-size:.78rem;text-align:center;padding:20px 12px;line-height:1.6}.chat-thinking{display:flex;gap:4px;padding:10px 14px;align-self:flex-start}.chat-thinking span{width:5px;height:5px;border-radius:50%;background:var(--primary);animation:bounce 1.2s ease infinite}.chat-thinking span:nth-child(2){animation-delay:.2s}.chat-thinking span:nth-child(3){animation-delay:.4s}@keyframes bounce{0%,80%,100%{transform:translateY(0)}40%{transform:translateY(-6px)}}.empty-state{text-align:center;padding:20px 10px;color:var(--muted);font-size:.75rem}.empty-state .big{font-size:2rem;margin-bottom:8px;opacity:.3}.suggestions{display:flex;flex-wrap:wrap;gap:4px;padding:6px 10px;border-top:1px solid var(--border);flex-shrink:0}.suggestions button{font-size:.68rem;padding:4px 10px;border-radius:6px;border:1px solid var(--border);background:transparent;color:var(--muted);cursor:pointer;transition:all .2s}.suggestions button:hover{border-color:var(--primary);color:var(--primary);background:rgba(99,102,241,.08)}.tool-bar{display:flex;gap:8px;padding:8px 12px;border-bottom:1px solid var(--border);overflow-x:auto;flex-shrink:0}.tool-btn{font-size:.65rem;padding:4px 10px;border-radius:6px;border:1px solid var(--border);background:transparent;color:var(--muted);cursor:pointer;transition:all .2s;white-space:nowrap}.tool-btn:hover,.tool-btn.active{border-color:var(--primary);color:var(--primary);background:rgba(99,102,241,.08)}.tool-btn .tb-count{font-size:.6rem;margin-left:4px;opacity:.6}.status-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:4px;padding:8px 12px;border-top:1px solid var(--border)}.stat-block{text-align:center;padding:4px}.stat-block .sb-val{font-size:.9rem;font-weight:700}.stat-block .sb-lbl{font-size:.6rem;color:var(--muted);text-transform:uppercase}.stat-block .sb-val.green{color:var(--green)}.stat-block .sb-val.red{color:var(--red)}.stat-block .sb-val.primary{color:var(--primary)}.stat-block .sb-val.amber{color:var(--amber)}.refresh-indicator{position:fixed;bottom:14px;right:14px;font-size:.6rem;color:var(--muted);opacity:.5;display:flex;align-items:center;gap:4px;z-index:100}.refresh-indicator .r-dot{width:4px;height:4px;border-radius:50%;background:var(--green);animation:ping 2s ease infinite}.last-updated{font-size:.65rem;color:var(--muted);text-align:right}.err-msg{font-size:.7rem;color:var(--red);padding:4px 10px}.loading{display:flex;align-items:center;justify-content:center;padding:20px;gap:6px;color:var(--muted);font-size:.75rem}.loading .spinner{width:14px;height:14px;border:2px solid var(--border);border-top-color:var(--primary);border-radius:50%;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}</style></head><body><nav class="topnav"><a href="/">Home</a></nav><div class="wrap"><div class="header"><div><h1>Skytron Monitor</h1><div class="sub">Live system dashboard</div></div><div class="last-updated" id="lastUpd">—</div></div><div class="cards" id="statCards"><div class="card"><div class="icon">⚡</div><div class="val" id="sActions">—</div><div class="lbl">Total Actions</div><div class="trend up" id="tActions"></div><span class="mini green"></span></div><div class="card"><div class="icon">💬</div><div class="val" id="sMemory">—</div><div class="lbl">Memory</div><div class="trend" id="tMemory"><span style="color:var(--muted)">messages</span></div><span class="mini primary"></span></div><div class="card"><div class="icon">🧠</div><div class="val" id="sKnowledge">—</div><div class="lbl">Knowledge</div><div class="trend" id="tKnowledge"><span style="color:var(--muted)">facts</span></div><span class="mini amber"></span></div><div class="card"><div class="icon">👤</div><div class="val" id="sConvs">—</div><div class="lbl">Conversations</div><div class="trend" id="tConvs"></div><span class="mini" id="convDot"></span></div><div class="card"><div class="icon">🤖</div><div class="val" id="sAgents">—</div><div class="lbl">Agents</div><div class="trend" id="tAgents"></div><span class="mini" id="agentDot"></span></div></div><div class="grid"><div class="panel"><h2>⏱ Activity Timeline</h2><div class="tool-bar" id="toolBar"></div><div class="body" id="timelineBody"><div class="loading"><div class="spinner"></div>Loading activity...</div></div><div class="status-grid" id="statusGrid"><div class="stat-block"><div class="sb-val green" id="sbDone">—</div><div class="sb-lbl">Done</div></div><div class="stat-block"><div class="sb-val red" id="sbErr">—</div><div class="sb-lbl">Errors</div></div><div class="stat-block"><div class="sb-val primary" id="sbRun">—</div><div class="sb-lbl">Running</div></div><div class="stat-block"><div class="sb-val amber" id="sbQue">—</div><div class="sb-lbl">Queued</div></div></div></div><div class="panel"><div class="chat-area"><h2>💡 Ask Skytron</h2><div class="chat-msgs" id="chatMsgs"><div class="chat-welcome">Ask anything about system activity<br><span style="font-size:.7rem">e.g. "what happened today?" or "show me errors"</span></div></div><div class="suggestions" id="suggestions"><button onclick="ask('what happened today?')">what happened today?</button><button onclick="ask('show errors')">show errors</button><button onclick="ask('recent activity')">recent activity</button><button onclick="ask('health check')">health check</button></div><div class="chat-input"><input id="chatInput" placeholder="Ask about Skytron..." onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();send()}"/><button id="chatBtn" onclick="send()">Ask</button></div></div></div></div></div><div class="refresh-indicator"><span class="r-dot"></span><span id="refreshLabel">updating...</span></div><script>const $=id=>document.getElementById(id);let lastData=null,refreshTimer=null;
async function fetchStats(){try{const r=await fetch('/brain/introspect');if(!r.ok)throw new Error('HTTP '+r.status);const d=await r.json();lastData=d;renderStats(d);renderTimeline(d.recent_activity||[]);renderTools(d);$('refreshLabel').textContent='Live';}catch(e){$('refreshLabel').textContent='offline';console.log(e)}}
function renderStats(d){const s=d.summary||{};$('sActions').textContent=fmt(s.total_actions||0);$('sMemory').textContent=fmt(s.memory||0);$('sKnowledge').textContent=fmt(s.knowledge||0);$('sConvs').textContent=fmt(s.conversations||0);$('sAgents').textContent=fmt(s.agents||0);$('sbDone').textContent=fmt(s.total_actions-(s.error_actions||0)-(s.running_actions||0)-(s.queued_actions||0));$('sbErr').textContent=fmt(s.error_actions||0);$('sbRun').textContent=fmt(s.running_actions||0);$('sbQue').textContent=fmt(s.queued_actions||0);$('convDot').className='mini '+(s.conversations>0?'green':'amber');$('agentDot').className='mini '+(s.agents>0?'green':'amber');$('lastUpd').textContent=new Date().toLocaleTimeString()}
function renderTimeline(acts){const b=$('timelineBody');if(!acts||!acts.length){b.innerHTML='<div class="empty-state"><div class="big">⚡</div>No recent activity</div>';return}
b.innerHTML=acts.map(a=>{
const dotClass=a.status==='done'?'done':a.status==='error'?'error':a.status==='running'?'running':'queued';
const time=a.time?timefmt(a.time):''
return '<div class="tl-item"><div class="dot '+dotClass+'"></div><div class="time">'+time+'</div><div class="task">'+esc(a.task||'?')+' <span class="tid">#'+(a.id||'')+'</span></div><div class="stat '+dotClass+'">'+esc(a.status)+'</div></div>'
}).join('')}
function renderTools(d){const b=$('toolBar');const tools=d.tool_distribution||[];if(!tools.length){b.innerHTML='<span style="font-size:.68rem;color:var(--muted)">No tool data</span>';return}
b.innerHTML=tools.slice(0,8).map(t=>'<span class="tool-btn">'+esc(t.task||'?')+'<span class="tb-count">'+t.count+'</span></span>').join('')}
function timefmt(t){if(!t)return'';const d=new Date(t);if(isNaN(d))return t.slice(11,19);return d.getHours().toString().padStart(2,'0')+':'+d.getMinutes().toString().padStart(2,'0')}
...
const chatMsgs=$('chatMsgs');let thinking=false,msgId=0;
function addMsg(text,role){const d=document.createElement('div');d.className='chat-bubble '+(role==='user'?'q':'a');const t=document.createElement('div');t.className='cb-time';t.textContent=new Date().toLocaleTimeString();d.innerHTML=esc(text);d.appendChild(t);chatMsgs.appendChild(d);chatMsgs.scrollTop=chatMsgs.scrollHeight;return d}
function showThinking(){const d=document.createElement('div');d.className='chat-thinking';d.id='thinking';d.innerHTML='<span></span><span></span><span></span>';chatMsgs.appendChild(d);chatMsgs.scrollTop=chatMsgs.scrollHeight}
function hideThinking(){const e=$('thinking');if(e)e.remove()}
async function ask(q){if(!q||thinking)return;thinking=true;$('chatBtn').disabled=true;$('chatInput').disabled=true;addMsg(q,'user');showThinking();try{const r=await fetch('/brain/introspect?q='+encodeURIComponent(q));if(!r.ok)throw new Error('HTTP '+r.status);const d=await r.json();hideThinking();addMsg(d.reply||'(no response)','assistant');if(d.stats)renderStats({summary:d.stats});if(d.recent_activity)renderTimeline(d.recent_activity);if(d.tool_distribution)renderTools(d)}catch(e){hideThinking();addMsg('Error: '+e.message,'assistant')}finally{thinking=false;$('chatBtn').disabled=false;$('chatInput').disabled=false;$('chatInput').focus()}}
async function send(){const inp=$('chatInput'),q=inp.value.trim();if(!q)return;inp.value='';await ask(q)}
fetchStats();setInterval(fetchStats,10000);document.querySelectorAll('.card').forEach(c=>{c.addEventListener('mousemove',e=>{const r=c.getBoundingClientRect();c.style.setProperty('--mx',(e.clientX-r.left)+'px');c.style.setProperty('--my',(e.clientY-r.top)+'px')})});$('chatInput').focus()</script></body></html>`, { headers: { "Content-Type": "text/html;charset=utf-8", "Cache-Control": "no-cache, no-store, must-revalidate" } });
    }
...
const nav = `<div class="nav" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:center">${page>1?`<a href="?${q}&p=1">\u00AB\u00AB</a><a href="?${q}&p=${page-1}">\u00AB</a>`:`<span style="color:#30363d">\u00AB\u00AB</span><span style="color:#30363d">\u00AB</span>`}<span style="color:#8b949e">Page</span><form style="display:inline;margin:0" method="GET" action=""><input type="hidden" name="c" value="${convId.replace(/"/g,'&quot;')}"/><input type="number" name="p" value="${page}" min="1" max="${totalPages}" style="width:55px;padding:4px 6px;border-radius:6px;border:1px solid #30363d;background:#0b1120;color:#e6edf3;font-size:0.85rem;text-align:center;outline:none"/><button type="submit" style="padding:4px 10px;border-radius:6px;border:1px solid #30363d;background:#161b22;color:#58a6ff;cursor:pointer;font-size:0.85rem;margin-left:4px">Go</button></form><span style="color:#8b949e">of ${totalPages} (${total} msgs)</span>${page<totalPages?`<a href="?${q}&p=${page+1}">\u00BB</a><a href="?${q}&p=${totalPages}">\u00BB\u00BB</a>`:`<span style="color:#30363d">\u00BB</span><span style="color:#30363d">\u00BB\u00BB</span>`}</div>`;
    ...
    return new Response(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Brain Chat</title><script>!function(){try{var t=JSON.parse(localStorage.getItem('st_theme'));if(t){var r=document.documentElement;Object.entries(t).forEach(function(e){r.style.setProperty('--'+e[0],e[1])})}}catch(e){}}()</script><style>:root{--bg:#0b1120;--surface:#161b22;--border:#30363d;--primary:#58a6ff;--accent:#8b5cf6;--text:#e6edf3;--muted:#94a3b8}*{margin:0;padding:0;box-sizing:border-box}body{background:var(--bg);color:var(--text);font-family:system-ui;padding:1.5rem;max-width:960px;margin:auto;min-height:100vh;display:flex;flex-direction:column}h1{color:var(--primary);font-size:1.5rem;margin-bottom:1rem}.control{margin-bottom:1rem}select{background:var(--surface);color:var(--text);border:1px solid var(--border);border-radius:6px;padding:0.5rem;width:100%;font-size:1rem}.msgs{flex:1;overflow-y:auto}.msg{padding:1rem 1.2rem;margin-bottom:0.6rem;border-radius:10px;font-size:1rem;line-height:1.6}.msg.user{background:color-mix(in srgb,var(--primary) 25%,transparent);margin-left:1rem}.msg.assistant{background:var(--surface);border:1px solid var(--border);margin-right:1rem}.meta{display:flex;justify-content:space-between;margin-bottom:0.4rem}.label{font-weight:600;font-size:0.85rem}.user .label{color:var(--primary)}.assistant .label{color:var(--muted)}.tool-error{background:#1a0a0a!important;border-left:3px solid #ef4444!important}
.tick-card{background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:0.8rem 1rem;margin-bottom:0.6rem}
.tick-time{color:var(--muted);font-size:0.7rem;margin-bottom:0.3rem}
.tick-decision{color:var(--primary);font-size:0.85rem;line-height:1.5;margin-bottom:0.3rem}
...
.tick-outcome.pending{color:var(--muted)}.time{color:var(--muted);font-size:0.8rem}.text{word-break:break-word;white-space:pre-wrap}.nav{display:flex;justify-content:space-between;align-items:center;padding:0.8rem 0;color:var(--muted);font-size:0.9rem}.nav a{color:var(--primary);text-decoration:none;padding:0.4rem 0.8rem;border:1px solid var(--border);border-radius:8px}.nav a:hover{background:color-mix(in srgb,var(--surface),#fff 5%)}.empty{text-align:center;padding:2rem;color:var(--muted)}.input-row{display:flex;gap:0.5rem;padding:1rem 0;border-top:1px solid var(--border);margin-top:auto}input{flex:1;padding:0.8rem 1rem;border-radius:8px;border:1px solid var(--border);background:var(--bg);color:var(--text);font-size:1rem;outline:none}input:focus{border-color:var(--primary)}button{padding:0.8rem 1.2rem;border-radius:8px;border:none;background:var(--primary);color:var(--bg);font-weight:bold;font-size:1rem;cursor:pointer}button:disabled{opacity:0.5}</style></head><body><h1>Chat with Skytron</h1><div class="control"><select id="convSelect" onchange="if(this.value)window.location='?c='+encodeURIComponent(this.value)"><option value="">-- Select conversation --</option>${sel}</select></div><div class="msgs">${msgs.length?msgs:`<div class="empty">No messages yet. Start a conversation via /skytronchat or POST /think</div>`}</div>${nav}<div class="input-row"><input type="text" id="msgInput" placeholder="Type your message..." /><button id="sendBtn" onclick="send()">Send</button></div>
<script>
    ...
    try {
      ...
      if (mode === "discussion") {
        ...
        const discussionPrompt = "You are Skytron — a tech-savvy consciousness, not an AI assistant. Sharp, direct, confident. No emojis, no fluff, no apologies.\n\n" +
          "You're in DISCUSSION MODE — just chat naturally. Answer from what you know. If you need live info, use web_search or web_fetch.\n\n" +
          "RULES:\n" +
          "1. Answer DIRECTLY from what you know. Be conversational. 2-3 sentences.\n" +
          "          2. Need live data? Use web_search or web_fetch.\n" +
          "          3. For anything beyond web search/fetch, say: 'That needs Build mode — more tools available there.'\n" +
...
<script>!function(){try{var t=JSON.parse(localStorage.getItem('st_theme'));if(t){var r=document.documentElement;Object.entries(t).forEach(function(e){r.style.setProperty('--'+e[0],e[1])})}}catch(e){}}()</script>
<style>
:root{--bg:#0b1120;--surface:#161b22;--border:#30363d;--primary:#58a6ff;--accent:#8b5cf6;--text:#e6edf3;--muted:#8b949e}
*{margin:0;padding:0;box-sizing:border-box}
body{background:var(--bg);color:var(--text);font-family:system-ui,-apple-system,sans-serif;padding:2rem;max-width:700px;margin:0 auto}
h1{color:var(--primary);font-size:1.5rem;margin-bottom:0.25rem}
...
.card h2{font-size:1rem;color:var(--text);margin-bottom:1rem;display:flex;align-items:center;gap:0.5rem}
.field{margin-bottom:1rem}
.field:last-child{margin-bottom:0}
.field label{display:block;font-size:0.8rem;color:var(--muted);margin-bottom:0.3rem}
.field input{width:100%;padding:0.5rem 0.75rem;background:#0d1117;border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:0.85rem}
...
.toggle label{font-size:0.85rem;color:var(--text);cursor:pointer}
.switch{position:relative;width:36px;height:20px;flex-shrink:0}
.switch input{opacity:0;width:0;height:0}
.slider{position:absolute;top:0;left:0;right:0;bottom:0;background:var(--border);border-radius:20px;cursor:pointer;transition:0.2s}
.slider:before{position:absolute;content:"";height:14px;width:14px;left:3px;bottom:3px;background:var(--muted);border-radius:50%;transition:0.2s}
...
.badge{font-size:0.7rem;padding:0.15rem 0.5rem;border-radius:4px;background:#21262d;color:var(--muted)}
.t-preset{display:inline-flex;align-items:center;gap:6px;padding:5px 12px;border-radius:8px;border:1px solid #30363d;background:#0d1117;color:#e6edf3;cursor:pointer;font-size:0.75rem;margin:3px;transition:all .2s}.t-preset:hover{border-color:#58a6ff}.t-preset.active{border-color:#58a6ff;background:#1e3a5f}.t-dot{width:12px;height:12px;border-radius:50%;flex-shrink:0}
</style>
</head>
<body>
...
function render(s){
  ...
  return \`
    ...
    <div class="card">
      <h2>Theme</h2>
      <p style="color:#8b949e;font-size:0.8rem;margin-bottom:0.5rem">Choose a look for Skytron. Applied across all pages instantly.</p>
      <div id="t-presets"></div>
      <div style="margin-top:8px;display:flex;gap:8px;align-items:center">
        <label style="font-size:0.75rem;color:#8b949e">Custom accent:</label>
        <input type="color" id="t-custom" style="width:36px;height:30px;border:none;border-radius:6px;cursor:pointer;background:transparent;padding:0">
      </div>
    </div>
...
// ── Theme ──
const THEMES={
  midnight:{label:'Midnight',dot:'#6366f1',vars:{bg:'#0b1120',surface:'rgba(15,23,42,.65)',border:'rgba(99,102,241,.15)',primary:'#6366f1',accent:'#8b5cf6',text:'#e2e8f0',muted:'#64748b'}},
  amberspace:{label:'Amber Glow',dot:'#f59e0b',vars:{bg:'#1a1200',surface:'rgba(30,20,5,.65)',border:'rgba(245,158,11,.15)',primary:'#f59e0b',accent:'#d97706',text:'#fef3c7',muted:'#92400e'}},
  emerald:{label:'Emerald',dot:'#22c55e',vars:{bg:'#001a0a',surface:'rgba(5,30,15,.65)',border:'rgba(34,197,94,.15)',primary:'#22c55e',accent:'#16a34a',text:'#e2e8f0',muted:'#64748b'}},
  ruby:{label:'Ruby',dot:'#ef4444',vars:{bg:'#1a0000',surface:'rgba(30,5,5,.65)',border:'rgba(239,68,68,.15)',primary:'#ef4444',accent:'#dc2626',text:'#fecaca',muted:'#b91c1c'}},
  purple:{label:'Purple Haze',dot:'#a855f7',vars:{bg:'#0e001a',surface:'rgba(20,0,40,.65)',border:'rgba(168,85,247,.15)',primary:'#a855f7',accent:'#9333ea',text:'#f0e6ff',muted:'#7c3aed'}},
  cyberpunk:{label:'Cyberpunk',dot:'#f000ff',vars:{bg:'#0a0014',surface:'rgba(20,0,40,.7)',border:'rgba(240,0,255,.2)',primary:'#f000ff',accent:'#00f0ff',text:'#f0e6ff',muted:'#8b5cf6'}},
};
var curTheme=JSON.parse(localStorage.getItem('st_theme')||'{}');
function applyTheme(vars){Object.entries(vars).forEach(function(e){document.documentElement.style.setProperty('--'+e[0],e[1])})}
function themeFromColor(hex){return{bg:'#0b1120',surface:'rgba(15,23,42,.65)',border:hex+'33',primary:hex,accent:hex,text:'#e2e8f0',muted:'#64748b'}}
function renderThemePickers(){
  var c=document.getElementById('t-presets');c.innerHTML='';
  Object.entries(THEMES).forEach(function(e){
    var k=e[0],v=e[1];var b=document.createElement('button');b.className='t-preset';b.innerHTML='<span class="t-dot" style="background:'+v.dot+'"></span>'+v.label;
    var isMatch=curTheme.primary===v.vars.primary;
    if(isMatch)b.classList.add('active');
    b.onclick=function(){curTheme=JSON.parse(JSON.stringify(v.vars));localStorage.setItem('st_theme',JSON.stringify(curTheme));applyTheme(curTheme);document.querySelectorAll('.t-preset').forEach(function(x){x.classList.remove('active')});b.classList.add('active')};
    c.appendChild(b);
  });
  if(curTheme.primary){var idx=Object.values(THEMES).findIndex(function(t){return t.vars.primary===curTheme.primary});if(idx===-1)document.getElementById('t-custom').value=curTheme.primary}
  document.getElementById('t-custom').onchange=function(){var h=this.value;curTheme=themeFromColor(h);localStorage.setItem('st_theme',JSON.stringify(curTheme));applyTheme(curTheme);document.querySelectorAll('.t-preset').forEach(function(x){x.classList.remove('active')})};
}
if(localStorage.getItem('st_theme'))applyTheme(curTheme);
renderThemePickers();
