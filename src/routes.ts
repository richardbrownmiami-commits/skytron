...
async function send(){var t=inp.value.trim();if(!t)return;var conv=document.getElementById('convSelect').value;inp.value='';btn.disabled=true;btn.textContent='...';try{var r=await fetch('/think?c='+encodeURIComponent(conv),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({input:t})});var d=await r.json();location.reload()}catch(e){btn.disabled=false;btn.textContent='Send'}}
  ...
  if (url.pathname === "/brain/repair-dashboard" && req.method === "GET") {
    return new Response(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Skytron Repair</title><style>*{margin:0;padding:0;box-sizing:border-box}body{background:#0b1120;color:#e6edf3;font-family:system-ui;min-height:100vh;display:flex;flex-direction:column;padding:8px}[v-cloak]{display:none}.heartbeat-bar{height:3px;border-radius:0 0 4px 4px;transition:all 0.5s;margin-bottom:8px}.sub-tabs{display:flex;gap:0;margin-bottom:10px;background:#161b22;border-radius:6px;overflow:hidden}.sub-tab{padding:8px 20px;cursor:pointer;color:#8b949e;font-size:12px;font-weight:500;border-bottom:2px solid transparent;user-select:none}.sub-tab.active{color:#58a6ff;border-bottom-color:#58a6ff;background:#1c2333}.sub-tab:hover{color:#e6edf3;background:#1c2333}.llm-box{background:#161b22;border:1px solid #30363d;border-radius:8px;padding:10px;margin-bottom:10px}.llm-header{font-size:12px;font-weight:600;color:#58a6ff;margin-bottom:6px}.llm-row{padding:5px 0;border-bottom:1px solid #21262d;display:flex;flex-wrap:wrap;align-items:center;gap:4px 8px;font-size:12px}.llm-row:last-child{border:none}.llm-status-ic{font-size:10px}.llm-name{color:#e6edf3;font-weight:500}.llm-role{font-size:10px;padding:1px 6px;border-radius:3px;text-transform:uppercase;font-weight:600}.llm-primary{background:#1a3a2a;color:#3fb950}.llm-fallback{background:#1e2d3d;color:#58a6ff}.llm-tertiary{background:#2d1f1a;color:#d29922}.llm-model{color:#8b949e;font-size:11px}.llm-error{color:#f85149;font-size:11px;width:100%}.llm-source{color:#6b7280;font-size:10px;width:100%}.llm-footer{font-size:11px;color:#8b949e;padding:4px 0 0;border-top:1px solid #21262d;margin-top:4px}.ep-table{background:#161b22;border:1px solid #30363d;border-radius:8px;overflow:hidden}.ep-header{padding:8px 12px;font-size:12px;font-weight:600;color:#8b949e;background:#0b1120;border-bottom:1px solid #30363d}.ep-row{display:flex;align-items:center;padding:6px 12px;border-bottom:1px solid #21262d;font-size:12px;gap:8px}.ep-row:last-child{border:none}.ep-status{font-size:10px;width:16px}.ep-path{color:#e6edf3;flex:1;font-family:monospace;font-size:11px}.ep-badge{font-size:10px;padding:1px 6px;border-radius:3px;font-weight:500}.ep-ok{background:#0f2d1a;color:#3fb950}.ep-error{background:#2d1215;color:#f85149}.ep-limited{background:#2d1f1a;color:#d29922}.ep-off{background:#1c2333;color:#6b7280}.ep-source{color:#6b7280;font-size:10px;width:90px;text-align:right}.report-view{padding:0}.issues-section{background:#161b22;border:1px solid #30363d;border-radius:8px;padding:10px;margin-bottom:10px}.section-title{font-size:12px;font-weight:600;color:#8b949e;margin-bottom:6px}.section-title-sm{font-size:11px;font-weight:600;color:#8b949e;margin-bottom:4px}.empty-state{color:#6b7280;text-align:center;padding:20px;font-size:12px}.empty-state-sm{color:#6b7280;text-align:center;padding:10px;font-size:11px}.issue-card{padding:8px 10px;border:1px solid #30363d;border-radius:6px;margin-bottom:6px;cursor:pointer;transition:border-color 0.2s}.issue-card:hover{border-color:#58a6ff}.issue-error{border-left:3px solid #f85149}.issue-warning{border-left:3px solid #d29922}.issue-header{display:flex;align-items:center;gap:8px}.issue-code{font-size:10px;font-weight:700;color:#58a6ff;letter-spacing:0.5px;background:#1c2333;padding:1px 5px;border-radius:3px}.issue-sev{font-size:10px}.issue-title{font-size:12px;color:#e6edf3;flex:1}.issue-meta{font-size:10px;color:#6b7280;margin-top:3px;padding-left:20px}.detail-box{background:#161b22;border:1px solid #30363d;border-radius:8px;padding:12px;margin-bottom:10px}.detail-empty{color:#6b7280;text-align:center;padding:20px;font-size:11px}.detail-code{font-size:11px;font-weight:700;color:#58a6ff;margin-bottom:2px}.detail-title{font-size:14px;color:#e6edf3;margin-bottom:8px;font-weight:500}.detail-body{background:#0b1120;border-radius:6px;padding:8px;margin-bottom:6px}.detail-row{display:flex;padding:3px 0;gap:10px;font-size:12px;border-bottom:1px solid #21262d}.detail-row:last-child{border:none}.detail-label{color:#8b949e;width:80px;flex-shrink:0}.detail-val{color:#e6edf3;flex:1}.detail-desc{font-size:11px;color:#8b949e;line-height:1.5;padding:4px 0}.fix-row{display:flex;gap:8px;margin-bottom:10px}.fix-col{flex:1;background:#161b22;border:1px solid #30363d;border-radius:8px;padding:10px}.fix-item{padding:5px 0;border-bottom:1px solid #21262d;font-size:11px;display:flex;gap:6px;align-items:flex-start}.fix-item:last-child{border:none}.fix-icon{flex-shrink:0;font-size:11px;margin-top:1px}.fix-auto .fix-icon{color:#3fb950}.fix-skytron .fix-icon{color:#d29922}.fix-text{color:#e6edf3;flex:1}.fix-source{color:#6b7280;font-size:9px;text-align:right;width:80px}.action-bar{display:flex;align-items:center;gap:10px;padding:10px;background:#161b22;border:1px solid #30363d;border-radius:8px;margin-bottom:8px;flex-wrap:wrap}.btn{padding:8px 18px;border-radius:6px;border:none;font-size:12px;font-weight:600;cursor:pointer;white-space:nowrap}.btn:disabled{opacity:0.5;cursor:not-allowed}.btn-checkup{background:#30363d;color:#e6edf3}.btn-checkup:hover:not(:disabled){background:#3b4252}.btn-fix{background:#238636;color:#fff}.btn-fix:hover:not(:disabled){background:#2ea043}.checkup-status{display:flex;align-items:center;gap:6px;flex:1}.checkup-spinner{width:14px;height:14px;border:2px solid #30363d;border-top-color:#58a6ff;border-radius:50%;animation:spin 0.6s linear infinite}.checkup-msg{font-size:11px}.checkup-idle{color:#8b949e}.checkup-checking{color:#58a6ff}.checkup-complete{color:#3fb950}.checkup-error{color:#f85149}.checkup-fixing{color:#d29922}.fix-results{background:#161b22;border:1px solid #30363d;border-radius:8px;padding:10px}.fix-res-item{padding:4px 0;font-size:11px;display:flex;gap:6px;align-items:center;border-bottom:1px solid #21262d}.fix-res-item:last-child{border:none}.fix-res-item .fix-icon{color:#3fb950}.fix-when{color:#6b7280;font-size:9px;margin-left:auto}.status-view{padding:0;overflow-y:auto}@keyframes spin{to{transform:rotate(360deg)}}</style></head><body><div id="repairApp" v-cloak style="flex:1;display:flex;flex-direction:column;overflow:hidden"></div><script type="importmap">{"imports":{"vue":"https://unpkg.com/vue@3/dist/vue.esm.browser.prod.js"}}</script><script type="module">import{createApp,ref,computed,onMounted}from'vue';createApp({setup(){const issues=ref([]),endpoints=ref([]),llmProviders=ref([]),llmPrimary=ref(''),llmFallback=ref(''),llmWorking=ref(false),activeSubTab=ref('status'),selectedIssue=ref(null),checkupStatus=ref('idle'),checkupMessage=ref(''),needsFix=ref(false),autoResolved=ref([]),skytronFixes=ref([]),heartbeat=ref('green'),loading=ref(true),fixRunning=ref(false),lastFixData=ref(null);function hc(){const h={green:'#3fb950',orange:'#d29922',red:'#f85149'};return h[heartbeat.value]||'#3fb950'}async function ld(){loading.value=true;try{const r=await fetch('/brain/health-check'),d=await r.json();issues.value=d.issues||[];endpoints.value=d.endpoints||[];llmProviders.value=d.llm?.providers||[];llmPrimary.value=d.llm?.primary||'none';llmFallback.value=d.llm?.fallback||'none';llmWorking.value=d.llm?.working||false;autoResolved.value=d.auto_resolved||[];skytronFixes.value=d.skytron_fixes||[];heartbeat.value=d.heartbeat||'green';needsFix.value=issues.value.length>0;checkupStatus.value='complete';checkupMessage.value=issues.value.length?issues.value.length+' issue(s) found':'All healthy'}catch(e){checkupStatus.value='error';checkupMessage.value='Failed to fetch'}loading.value=false}async function rc(){checkupStatus.value='checking';checkupMessage.value='Scanning...';selectedIssue.value=null;await ld()}async function rf(){fixRunning.value=true;checkupStatus.value='fixing';checkupMessage.value='Applying fixes...';try{const r=await fetch('/brain/repair',{method:'POST'}),d=await r.json();lastFixData.value=d;await ld();checkupStatus.value='complete';checkupMessage.value=issues.value.length?issues.value.length+' issue(s) remaining':'All resolved'}catch(e){checkupStatus.value='error';checkupMessage.value='Fix failed'}fixRunning.value=false}function si(s){return s==='ok'?'\u{1F7E2}':s==='warning'||s==='limited'?'\u{1F7E1}':s==='error'?'\u{1F534}':'\u26AA'}function se(s){return s==='error'?'\u{1F534}':s==='warning'?'\u{1F7E1}':'\u{1F7E2}'}function ta(t){if(!t)return'';const d=new Date(t),n=new Date(),f=Math.floor((n-d)/1000);if(f<60)return f+'s ago';if(f<3600)return Math.floor(f/60)+'m ago';if(f<86400)return Math.floor(f/3600)+'h ago';return Math.floor(f/86400)+'d ago'}onMounted(()=>{ld()});return{issues,endpoints,llmProviders,llmPrimary,llmFallback,llmWorking,activeSubTab,selectedIssue,checkupStatus,checkupMessage,needsFix,autoResolved,skytronFixes,heartbeat,loading,fixRunning,lastFixData,heartbeatColor:hc,hasIssues:computed(()=>issues.value.length>0),selectIssue(i){selectedIssue.value=selectedIssue.value?.code===i.code?null:i},runCheckup:rc,runFix:rf,statusIcon:si,severityIcon:se,timeAgo:ta}},template:'<div class="heartbeat-bar" :style="{background: heartbeatColor, boxShadow: \'0 0 12px \'+heartbeatColor}"></div><div class="sub-tabs"><div :class="\'sub-tab\'+(activeSubTab===\'status\'?\' active\':\'\')" @click="activeSubTab=\'status\'">Status</div><div :class="\'sub-tab\'+(activeSubTab===\'report\'?\' active\':\'\')" @click="activeSubTab=\'report\'">Report</div></div><div v-show="activeSubTab===\'status\'" class="status-view"><div class="llm-box"><div class="llm-header">LLM Providers</div><div v-for="p in llmProviders" class="llm-row"><span class="llm-status-ic">{{statusIcon(p.status)}}</span><span class="llm-name">{{p.name}}</span><span :class="\'llm-role llm-\'+p.role">{{p.role}}</span><span class="llm-model">{{p.model}}</span><div v-if="p.error" class="llm-error">{{p.error}}</div><div class="llm-source">{{p.source_file}}</div></div><div class="llm-footer">Primary: <strong>{{llmPrimary}}</strong> &#183; Fallback: <strong>{{llmFallback}}</strong> &#183; {{llmWorking?\'Working\':\'No working provider\'}}</div></div><div class="ep-table"><div class="ep-header">All Endpoints</div><div v-for="ep in endpoints" class="ep-row"><span class="ep-status">{{statusIcon(ep.status)}}</span><span class="ep-path">{{ep.path}}</span><span :class="\'ep-badge ep-\'+ep.status">{{ep.status}}</span><span class="ep-source">{{ep.source}}</span></div></div></div><div v-show="activeSubTab===\'report\'" class="report-view"><div class="issues-section"><div class="section-title">Dev Repair Issues</div><div v-if="issues.length===0" class="empty-state">All healthy &#8212; no issues detected</div><div v-for="iss in issues" :key="iss.code" :class="\'issue-card issue-\'+iss.severity" @click="selectIssue(iss)"><div class="issue-header"><span class="issue-code">{{iss.code}}</span><span class="issue-sev">{{severityIcon(iss.severity)}}</span><span class="issue-title">{{iss.title}}</span></div><div class="issue-meta">{{iss.source_file}}</div></div></div><div v-if="selectedIssue" class="detail-box"><div class="detail-code">{{selectedIssue.code}}</div><div class="detail-title">{{selectedIssue.title}}</div><div class="detail-body"><div class="detail-row"><span class="detail-label">When</span><span class="detail-val">{{selectedIssue.when}}</span></div><div class="detail-row"><span class="detail-label">For how long</span><span class="detail-val">{{selectedIssue.duration}}</span></div><div class="detail-row"><span class="detail-label">Effect</span><span class="detail-val">{{selectedIssue.effect}}</span></div><div class="detail-row"><span class="detail-label">Source</span><span class="detail-val">{{selectedIssue.source_file}}</span></div></div><div class="detail-desc">{{selectedIssue.description}}</div></div><div v-else class="detail-box detail-empty">Click any issue above for details</div><div class="fix-row"><div class="fix-col"><div class="section-title-sm">Auto-Resolved (24h)</div><div v-if="autoResolved.length===0" class="empty-state-sm">None in last 24h</div><div v-for="f in autoResolved" class="fix-item fix-auto"><span class="fix-icon">&#10003;</span><span class="fix-text">{{f.description}}</span><span class="fix-source">{{f.source_file}}</span></div></div><div class="fix-col"><div class="section-title-sm">Fixed by Skytron</div><div v-if="skytronFixes.length===0" class="empty-state-sm">No recent self-fixes</div><div v-for="f in skytronFixes" class="fix-item fix-skytron"><span class="fix-icon">&#9889;</span><span class="fix-text">{{f.description}}</span><span class="fix-source">{{f.source_file}}</span></div></div></div><div class="action-bar"><button class="btn btn-checkup" @click="runCheckup" :disabled="checkupStatus===\'checking\'||checkupStatus===\'fixing\'">{{checkupStatus===\'fixing\'?\'Repairing...\':checkupStatus===\'checking\'?\'Scanning...\':\'Run Checkup\'}}</button><div class="checkup-status"><div v-if="checkupStatus===\'checking\'||checkupStatus===\'fixing\'" class="checkup-spinner"></div><span :class="\'checkup-msg checkup-\'+checkupStatus">{{checkupMessage}}</span></div><button v-if="needsFix" class="btn btn-fix" @click="runFix" :disabled="fixRunning||checkupStatus===\'fixing\'">Fix Issues</button></div><div v-if="lastFixData" class="fix-results"><div class="section-title-sm">Last Fix Results</div><div v-for="f in lastFixData.fixes" class="fix-res-item"><span class="fix-icon">&#9889;</span><span>{{f.description}}</span><span class="fix-source">{{f.source_file}}</span><span class="fix-when">{{timeAgo(f.when)}}</span></div></div></div>'}).mount('#repairApp')</script></body></html>`, { headers: { "Content-Type": "text/html;charset=utf-8", "Cache-Control": "no-cache, no-store, must-revalidate" } });
  }
