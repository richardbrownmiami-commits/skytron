...
export const SEED_KNOWLEDGE = [
  ...
  { k: "tool_one_knowledge", c: "one_knowledge(platform, action?, query?): looks up API documentation from One Knowledge API (76K+ API tools across 460 platforms). Platform is required (e.g. 'twitter', 'stripe', 'github'). Query is optional search term.", cat: "tools" },
   ...
   { k: "tool_github_get_file", c: "github_get_file(repo, path, branch?): reads a file. For YOUR repo ('richardbrownmiami-commits/skytron'), automatically checks brain_knowledge first — pass ANY path like 'src/tools.ts' or 'tools.ts'. No GitHub API needed for your own files. branch defaults to 'main'. Use this to read your own source files.", cat: "tools" },
